...
parser.add_argument('--input_dimension', default=[None,40,1])
parser.add_argument('--cnn1_dimension', default=[7,1,16])
parser.add_argument('--cnn2_dimension', default=[7,16,32])
parser.add_argument('--cnn3_dimension', default=[7,32,64])
parser.add_argument('--cnn4_dimension', default=[7,64,32])
...
train_x, train_y, test_x = data_loading.get_data_1d()
...
with tensorflow.device(flag.device):
    ...
    w5 = tensorflow.contrib.eager.Variable(tensorflow.truncated_normal((512, flag.fc5_dimension), stddev=0.1), name='w5')
...
def model(x, is_train):
    h1 = tensorflow.nn.leaky_relu(tensorflow.nn.conv1d(x, w1, stride=1, padding='VALID') + b1)

    h2 = tensorflow.nn.leaky_relu(tensorflow.nn.conv1d(h1, w2, stride=1, padding='VALID') + b2)

    h3 = tensorflow.nn.leaky_relu(tensorflow.nn.conv1d(h2, w3, stride=1, padding='VALID') + b3)

    h4 = tensorflow.nn.leaky_relu(tensorflow.nn.conv1d(h3, w4, stride=1, padding='VALID') + b4)
...
def main(_):
    ...
    pandas.DataFrame(predict.numpy()).to_csv('cnn_1d %s.csv'%numpy.around(accuracy.numpy()[0], decimals=1), index=False, header=False)
