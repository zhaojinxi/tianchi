...
parser.add_argument('--input_dimension', default=[None,7,7,1])
parser.add_argument('--cnn1_dimension', default=[3,3,1,8])
parser.add_argument('--cnn2_dimension', default=[3,3,8,16])
parser.add_argument('--cnn3_dimension', default=[3,3,16,32])
parser.add_argument('--cnn4_dimension', default=[3,3,32,16])
...
train_x, train_y, test_x = data_loading.get_data_2dimension()
...
w5 = tensorflow.contrib.eager.Variable(tensorflow.truncated_normal((64, flag.fc5_dimension), stddev=0.1), name='w5')
...
def main(_):
    ...
    pandas.DataFrame(predict.numpy()).to_csv('cnn_2d %s.csv'%numpy.around(accuracy.numpy()[0], decimals=1), index=False, header=False)
