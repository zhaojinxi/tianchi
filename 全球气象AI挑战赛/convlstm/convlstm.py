...
def convlstm_encode(c_old,h_old,x):
    with tensorflow.variable_scope('convlstm_encode', reuse=tensorflow.AUTO_REUSE):
        fxw=tensorflow.get_variable('fxw',[3,3,1,1])
        fhw=tensorflow.get_variable('fhw',[3,3,1,1])
        fcw=tensorflow.get_variable('fcw',[batch_size,501,501,1])
        fb=tensorflow.get_variable('fb',1)
        ...
        ixw=tensorflow.get_variable('ixw',[3,3,1,1])
        ihw=tensorflow.get_variable('ihw',[3,3,1,1])
        icw=tensorflow.get_variable('icw',[batch_size,501,501,1])
        ib=tensorflow.get_variable('ib',1)
        ...
        txw=tensorflow.get_variable('txw',[3,3,1,1])
        thw=tensorflow.get_variable('thw',[3,3,1,1])
        tb=tensorflow.get_variable('tb',1)
        ...
        oxw=tensorflow.get_variable('oxw',[3,3,1,1])
        ohw=tensorflow.get_variable('ohw',[3,3,1,1])
        ocw=tensorflow.get_variable('ocw',[batch_size,501,501,1])
        ob=tensorflow.get_variable('ob',1)
...
def model(input_encode):
    all_output_encode=[]
    init_cell=numpy.zeros([batch_size,501,501,1]).astype(numpy.float32)
    init_hide=numpy.zeros([batch_size,501,501,1]).astype(numpy.float32)
    for i in range(31):
        if i==0:
...
input_encode=tensorflow.placeholder(tensorflow.float32,[batch_size,61,501,501,1])
